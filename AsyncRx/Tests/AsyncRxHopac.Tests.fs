module AsyncRxHopacTests

(*
    AsyncRxHopac.Tests.fs
    ...
    Run:  dotnet run --project AsyncRx/Tests/AsyncRxHopac.Tests.fsproj
*)
...
let run () =
    ...
    let fail (e: exn) = AsyncObservable.fail e
    ...
    // as exactly one downstream error Ã¢â‚¬â€ the throw is caught and routed through the gate.
