...
## Design snapshot (living — last updated 2026-07-05)
...
**Surfaces — two faces over one vocabulary (2026-07-05, D12–D13).** With both D3 workloads now built
(Sudoku on `module M`, WFC on `module Wfc`), the surface is designable. The engine presents a *friendly*
face (clear rep `Set<int>`/any `'a`, closure props, rich lattices — clarity/debugging/dataflow; ≈
`module M` dressed) and an *optimized* face (flat-word rep, structural props, trail, cone-local `Retract`,
min-entropy — 60fps/cheap; ≈ `module Wfc` dressed), unified at **the D1 contract plus a rep-agnostic
authoring vocabulary** (ready domains, named cell/premise handles, constraint boxes, consume verbs) —
*not* a single storage engine (a universal flat `uint64[nCells*W]` cannot hold `Set<int>`; rejected). The
raw emit-closure becomes a compilation target, not the surface. Proposed spine: **write once, swap rep**
(one spec, two lowerings). Propagator **wiring** (closure vs structural) joins value-rep (D3) and store
(D10) as a third instantiation dial. The premise ceiling is a *per-backend representation* decision, not
contract (D14): v1 = one word + loud allocation failure; growth is friendly-face-local, on measured need.
Full design + fork statuses: [propagator-surface-design.md](propagator-surface-design.md).
...
| D12 | The engine presents **two named faces** over the D1 contract — a *friendly* face (clear rep `Set<int>`/any `'a`, closure props; ≈ `module M` dressed) and an *optimized* face (flat-word rep, structural props, trail, cone-local `Retract`, min-entropy; ≈ `module Wfc` dressed) — unified at **the contract + a rep-agnostic authoring vocabulary** (ready domains, named cell/premise handles, constraint boxes, consume verbs), **not** a single storage engine (a universal flat word store cannot hold `Set<int>`; rejected). The raw emit-closure is demoted to a compilation target. **Write once, swap rep** (one spec, two lowerings) is the proposed spine. Resolves the "Public capability surfaces" OQ; concretizes D2; extracts the D3 interface. Full design: [propagator-surface-design.md](propagator-surface-design.md). | Decided (write-once Tentative) | 2026-07-05 |
| D13 | Propagator **wiring** is a third instantiation dial, orthogonal to D3 (value rep) and D10 (store): **closure props** (arbitrary graph — Sudoku units, C↔F, dataflow) vs **structural props** (enumerated neighbors — the WFC grid, no per-edge closure). Independent of the other dials: Sudoku wants flat-word store *and* closure props. | Decided, tentative shape | 2026-07-05 |
| D14 | The **premise ceiling is a representation decision per backend, not a contract decision** (fork B closed). The contract's obligation is *support = finite premise set*, width unstated; premise handles hide the rep. v1 = one `uint64` word on both backends + **loud** premise-allocation failure at exhaustion (no silent aliasing). Growth path = P-word support masks (the ⌈n/64⌉ tile trick), **friendly-face-local**, adopted on measured need; the optimized face keeps one word (authored constraints only; multi-word would tax every narrow's support-OR at 250k cells). Excess premise count is a lower-time capability failure like any other. | Decided (Fable, Tier 0) | 2026-07-05 |
...
- **Public capability surfaces — answered 2026-07-05 ([propagator-surface-design.md](propagator-surface-design.md)).** Resolved into **two named faces** (friendly / optimized) over the D1 contract, unified by a rep-agnostic **vocabulary** (ready domains, named cell/premise handles, constraint boxes, consume verbs — D12), with propagator wiring as a third dial (D13). The shared artifact is the contract + vocabulary, not a single storage engine. A search module still sits above, unchanged (D2/D4). Three sub-questions promoted to their own bullets below (mechanism parity, premise ceiling, write-once commitment).
- **Mechanism parity on the friendly backend (fork A) — RATIFIED (Deen) 2026-07-05.** The premise ("the verbs *need* trail + `Collapse` + `OnChange`") was false: the friendly lowering of `solve`/`solutions`/`generate` is **dependency-directed backtracking on `module M` as-is** — guess = `Assert` under a fresh premise, backtrack = `Retract` of it; O(network) replay per backtrack and O(cells) scan picking are the friendly face's honest, acceptable costs. No mechanism port for v1; port only on measured friendly demand. D2's "same laws, different machinery" made operational; each search driver owns its own undo (D4 boundary intact on both faces). ([propagator-surface-design.md](propagator-surface-design.md) §10.)
- **Premise ceiling under the handle surface (fork B) — answered 2026-07-05 (Fable, Tier 0; D14).** De-escalated: the shared `uint64` was an implementation coincidence, not a law — the contract says only *support = finite premise set*. v1 = one word both backends + loud allocation failure; growth = friendly-face-local P-word supports on measured need; optimized face stays one word. ([propagator-surface-design.md](propagator-surface-design.md) §10.)
- **Write-once commitment (fork C) — RATIFIED (Deen) 2026-07-05: reading (b) locked**, with the commitment scoped as *portable constructs are authored once* (not *all constructs are portable*), made honest by lower-time capability gating. Falsifier during the proof slices: a portable constraint box needing backend-conditional branches in its *spec-level definition* means the discipline is failing — degrade to shared-laws-only explicitly, do not rescue. ([propagator-surface-design.md](propagator-surface-design.md) §10.)
...
### 2026-07-05 — Two faces over one vocabulary: the surface design
**Author:** Claude (Opus 4.8); Deen set the framing constraints, the design is mine.
**Status:** Accepted — adds D12 (two faces + shared vocabulary) and D13 (wiring axis); answers the "Public capability surfaces" open question; concretizes D2; extracts the D3 interface. Full design in [propagator-surface-design.md](propagator-surface-design.md). Four decided points, three forks left open. No prior decision reversed (D1/D5/D7 untouched).

The engine core is built (mutable-core steps 1–5; `module M` general core, `module Wfc` specialized store with trail / cone-local `Retract` / min-entropy search). Both D3 workloads now exist, so the surface — deferred until they did — is designable, and the journal's capability-surfaces question is answerable.

**The trigger.** Authoring a propagator on the general core means writing the engine's own bookkeeping at the call site: `e.AddProp([cC], fun emit -> match e.Value cC with Val x -> emit fF (Val (x*9./5.+32.)) (e.Support cC) | _ -> ())` names `cC` three times (read, value-fetch, provenance-fetch), hand-threads the support mask, hand-matches the not-yet-known case, couples the body to a specific engine `e`, and authors one direction of a symmetric relation with nothing tying it to its twin. The emit callback and the support bitmask are *substrate*; here the substrate is the surface (the A3 violation).

**The requirement (Deen).** One library with two faces: a *friendly* face where a simple `Set<int>` problem is a few lines, and an *optimized* face where a game-performant 60fps cheap WFC is expressible.

**The key resolution — unify above storage, not at it.** A single universal flat `uint64[nCells*W]` store was floated and **rejected**: it serves the optimized face but cannot hold a `Set<int>`, and forcing the clear rep into words is the opposite of friendly. So the unification is one semantic contract (D1) + one rep-agnostic authoring vocabulary + the rep as a *pluggable backend*. This also dissolves the word-dispatch benchmark risk a shared store would have carried: the optimized face keeps `module Wfc`'s specialized inlined store (its 60fps numbers stand), the friendly face keeps generic `Set<int>`, and they never share bytes — so nothing on the hot path pays for the friendly face.

**The vocabulary is the spine (D12).** Ready domains (finite ones lower to Set *or* words; rich ones like interval/scalar stay friendly-only concrete lattices — [propagator-number-types.fsx](../propagator-number-types.fsx) already built them), named/typed cell handles, premise handles (hiding allocation and the 64-bit ceiling), constraint boxes (`allDifferent`/`adjacency` carrying two lowerings; `convert`/`sum`/`equal` friendly-only, where the relation is written once and directionality + provenance are generated), and consume verbs (`solve`/`solutions`/`generate`). The raw emit-closure becomes a compilation target. Proposed spine: **write once, swap rep** — one rep-agnostic spec, two lowerings (friendly Set-engine + closure props; optimized word-store + structural props) — Tentative pending fork C.

**New third dial (D13).** Propagator wiring (closure vs structural) is orthogonal to value-rep (D3) and store (D10); Sudoku already needs flat-word store + closure props, proving independence.

**Forks left open** (per the doc §7): mechanism parity on the friendly backend (fork A — does `module M` gain trail/`Collapse`/`OnChange`/write-index, or does search stay optimized-face-only for v1); the premise ceiling as a contract decision (fork B); and whether write-once is truly committed (fork C). Next: sketch the vocabulary in concrete F# signatures, then lock A and C.

### 2026-07-05 — Tier-0 review of the surface design: fork B decided, A resolved, C scoped
**Author:** Claude (Fable 5, Tier 0)
**Status:** Accepted — adds D14 (premise ceiling de-escalated from contract to per-backend representation; fork B closed). Fork A resolved and fork C recommended-locked, both pending Deen's ratification. Codex's 2026-07-05 suggestion (surface doc §10) accepted with two sharpenings. Full review: [propagator-surface-design.md](propagator-surface-design.md) §10. No prior decision reversed; D2/D4 boundaries reaffirmed.

**Fork A dissolves under our own provenance.** The fork assumed the consume verbs require porting `module Wfc`'s search mechanisms (trail, `Collapse`, `OnChange`, write-index) into `module M`. They do not: the friendly lowering of `solve`/`solutions`/`generate` is **dependency-directed backtracking** — guess = `Assert` under a fresh premise, backtrack = `Retract` of that premise — which `module M` supports today. The trail is the optimized face's *specialization of undo*, not a prerequisite for search; replay-all `Retract` (O(network), microseconds at Sudoku scale) is the friendly face's honest cost. Verb-level parity, mechanism-level divergence — D2 made operational. One coupling: DDB spends one live premise per open guess, so friendly search depth shares the premise budget — which feeds directly into fork B's v1 requirement.

**Fork B (D14): the ceiling was never a contract question.** Both faces sharing `uint64` supports is an implementation coincidence; the contract obliges only *support = finite premise set*. v1 keeps one word on both backends and makes premise allocation **fail loudly** at exhaustion — the only v1 work. Growth (P-word supports, the ⌈n/64⌉ trick) is friendly-face-local and waits for measured need; the optimized face keeps one word indefinitely (authored constraints only; widening would tax every narrow's support-OR at 250k cells for no user).

**Fork C: lock write-once, scoped.** *Portable constructs are authored once* — not *everything is portable*; lower-time capability gating (Codex) is what keeps that honest. Falsifier declared: backend-conditional branches appearing in a portable construct's *spec-level definition* (not in its lowerings) mean the discipline is failing and should be degraded to shared-laws-only explicitly.

**Sharpenings of Codex's mechanics:** capability witnesses as ordinary lowering return types (`Friendly.lower : Spec -> Result<FriendlyNet, UnsupportedConstruct list>`, each net type exposing only its legal verbs; failures as values naming constructs), not phantom-type/SRTP proofs; and the Sudoku proof slice must run `solve` through both lowerings differentially — same spec, both faces, equal solutions — the two-faces claim in one assertion. Two vocabulary gaps flagged for the signature sketch: a `watch` observation verb (the AsyncRx north star; all current verbs are batch), and the runtime-edit vs structural-edit split (premise operations on the lowered net vs a new spec + re-lower; the interactive WFC goal lives entirely on the premise side).

### 2026-07-05 — Deen ratifies forks A and C; Opus builds the slice-0 vocabulary skeleton

**Author:** Claude (Opus 4.8), executing Deen's ratification.
**Status:** Accepted — forks A and C locked (fork B was already decided, D14). No decision reversed.

Deen ratified the two forks that were his to lock (surface doc §7): **A** — no mechanism port; the friendly consume verbs lower to dependency-directed backtracking on `module M` as-is. **C** — write-once, scoped to *portable constructs are authored once*. Both were resolved / recommended in the same-day Tier-0 (Fable) review; this entry records the lock, not new analysis.

The first executor artifact followed immediately: **[propagator-surface-vocab.fsx](../propagator-surface-vocab.fsx)**, the slice-0 signature skeleton (Codex build-order step 1, Fable sharpening 1). It pins the nouns (`Domain` finite/lattice, phantom-typed `Cell`, opaque `Premise` that names no word — fork B), the spine (`Constraint` = `Relation` portable / `Dataflow` friendly-only / `Structural` optimized-only), the write-once `Spec`, and the two lowerings `Friendly.lower` / `Optimized.lower : Spec -> Result<_, UnsupportedConstruct list>` — capability witnesses as ordinary return values, legality visible before any engine exists. Fork B's premise-width overflow rides the same witness channel (`PremiseWidthExceeded`). `Differential.solveBoth` encodes sharpening 2 (the two-faces claim in one call); the `watch` observation verb (gap 1) and the runtime-edit / structural-edit split (gap 2) are reserved in the surface so the noun/verb split will not need refitting. Both proof-slice specs (C↔F friendly-only, 4×4 Sudoku portable) are authored compile-only. Type-checks under fsi; every body is a `failwith` stub. Next: fill the slice bodies (surface doc §9.3).
