...
module General = Propagator.General
module Optimized = Propagator.Optimized
...
module private FiniteCoreRegression =
    ...
    let settledBaselineRefresh () =
        let makeNetwork () =
            let net = Propagator.Network.Domain.finite [ 1; 2; 3 ]
        ...
        let visible (net: Propagator.Network.Network<int, int, Set<int>>) cells =
        ...
        let failedRestartRestoresBaseline =
            let net = Propagator.Network.Domain.finite [ 1; 2 ]
    ...
    let binaryHotPathEdges () =
        let fullNet = Propagator.Network.Domain.finite [ 0; 1; 2 ]
        ...
        let emptyNet = Propagator.Network.Domain.finite [ 0; 1; 2 ]
    ...
    let multiwordObserverReentry () =
        let net = Propagator.Network.Domain.finite [ 0 .. 64 ]
...
if main () <> 0 then
    failwith "propagator regression suite failed"
